...
def insertData():
    ...
    DEDUCTIONS.set("")
    NET_PAY.set("")
    txt_result.config(text='Data Successfully Added!', fg='blue')
...
def updateData():
    ...
    cursor.execute("UPDATE `employee` SET `employee_no` = ?, `employee_name` = ?, `rate` = ?, `days` = ?, `gross` = "
                   ...
                   (str(EMPLOYEE_NO.get()), str(EMPLOYEE_NAME.get()), str(Rate.get()), str(Days.get()),
                    str(GROSS.get()),  str(SSS.get()), str(PHIL_HEALTH.get()), str(CA.get()), str(DEDUCTIONS.get()), str(NET_PAY.get()), int(mem_id)))
    conn.commit()
    cursor.execute("SELECT * FROM `employee` ORDER BY `employee_no` ASC")
    fetch = cursor.fetchall()
    for data in fetch:
    ...
    NET_PAY.set("")
    upwindow.destroy()
    txt_result.config(text='Data Successfully Updated!', fg='blue')
...
def update():
    if not tree.selection():
        ...
        update_button = Button(upwindow, width=12, height=2, text="Update Record", font=('arial', 10, 'bold'), bd=5, command=compute2,
                            cursor='hand2')
...
def add_record():
    ...
    # =====================LABEL ENTRY=======================
    Label(add, text="Adding Record", font=('times new roman', 15, 'bold'), bd=10, bg='powder blue').grid(row=0, columnspan=1)
    ...
    add_button = Button(add, width=12, height=2, text="Add Record", font=('arial', 10, 'bold'), bd=5, command=compute, cursor='hand2')
...
add_button = Button(Buttons, width=12, height=1, text="Add Record", bd=5, command=add_record, bg="#5BD0F0", cursor='hand2')
add_button.place(x=50)

update_button = Button(Buttons, width=12, height=1, text="Update Record", bd=5, command=update, state=DISABLED, bg="#5BD0F0", cursor='hand2')
update_button.place(x=180)

delete_button = Button(Buttons, width=12, height=1, text="Delete Record", bd=5, command=delete, state=DISABLED, bg="#5BD0F0", cursor='hand2')
